...
BatchCodeTest.scrapes.push({
    ...
    scrape: function(html) {
        const inputs  = Array.from(html.matchAll(/<h3>入力例 *\d+<\/h3>\s*<pre>\s*(.*?)\s*<\/pre>/gis), m => m[1].replaceAll("&lt;", "<").replaceAll("&gt;", ">"))
        const outputs = Array.from(html.matchAll(/<h3>出力例 *\d+<\/h3>\s*<pre>\s*(.*?)\s*<\/pre>/gis), m => m[1].replaceAll("&lt;", "<").replaceAll("&gt;", ">"))
        let input = undefined;
        const matched1 = html.match(/<h3>入力<\/h3>\s*<p>入力は以下の形式で(?:[^<]|<\/?var>)*<\/p>\s*<pre>(.*?)<\/pre>/s)
        if (matched1) {
