BatchCodeTest.make_input = function(_input, constraints) {
    ...
    while (input != "") {
        ...
        if (matched = input.match(/^\s*((.).*)\n(?:\2.*\n)*?.*\\vdots.*\n\2(?:[^_]*_\{?|\s*query\s*)([^},) ]+).*\n/i)) {
            ...
            let matched2
            if (matched2 = line.match(/^([a-z]).*[cl]?dots/i)) {
                // 2次元配列の場合
                const v = matched2[1]
            ...
            } else if (matched2 = line.match(/query/i)) {
                // クエリの場合
                if (constraints.match(/入力.*は(?:すべて|全て)整数/)) {
                    code += `query = [list(map(int, input().split())) for _ in range(${varname(n)})]\n`
                } else {
                    code += `query = [input().split() for _ in range(${varname(n)})]\n`
                }
