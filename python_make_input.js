        ...
        if (matched = input.match(/^\s*((.).*)\n(?:\2.*\n)*?.*(?::|︙|⋮|\\vdots).*\n\2(?:[^_]*_\{?|\s*query\s*(?:[^a-z]+\s*)?)([^},) ]+).*\n/i)) {
            ...
            if (matched2 = line.match(/^([a-z])_1 ([a-z])_\{1,\s*1\}.*(?:…|\.\.\.|[cl]?dots)/i)) {
                ...
                code += `    ${v1}[i], *${v2}[i] = list(map(int, input().split()))\n`
                       ...
                       (matched2 = line.match(/^([a-z])_\{1,\s*1\}\s*\1_\{1,\s*2\}/i))) {
                // 2次元配列の場合
                const v = matched2[1]
                let type = get_type(v)
                code += `${varname(v)} = `
                ...
                if (type == "str"  ) {
                    if (line.match(new RegExp(`\} +${v}`))) {
                        code += `[input().split() for _ in range(${varname(n)})]\n`
                    } else {
                        code += `[input() for _ in range(${varname(n)})]\n`
                    }
                }
                ...
                if (type == "str"  ) code += `[input() for _ in range(${varname(n)})]\n`
            } else {
                code += "# 最後まで入力コードの生成ができませんでした。ご注意ください。\n"
                break
            }
        } else if (matched = input.match(/^([a-z])[^a-z ]*\d\}?(?: +(?:\1|\\[cl]?dots|…|\.\.\.)\S*(?:\{[^\n\}]+\})?)* *\n/i)) {
            // 1行配列の場合
            const v = matched[1]
            const type = get_type(v)
            code += `${varname(v)} = `
            ...
            if (type == "str"  ) code += "input().split()\n"
        } else if (matched = input.match(/^([a-z]\S*(?: +\S+)+) *\n/i)) {
            // 1行複数変数の場合
